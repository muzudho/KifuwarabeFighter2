...
namespace DojinCircleGrayscale.Hitbox2DLorikeetMaker
{
    ...
    public class RectangleOnSlice
    {
        ...
        public RectangleOnSlice(int sliceX, int sliceY, int sliceWidth, int sliceHeight, int colliderX, int colliderY, int colliderWidth, int colliderHeight)
        {
        ...
        public float GetOffsetX(StringBuilder info_message)
        {
            ...
            x /= DataGenerator.UNITY_GRID_UNIT;
            if (info_message.Length < 10000)
            {
                info_message.AppendLine("GetOffsetX: x=[" + x + "] ((Collider.center.x=[" + Collider.center.x + "]) - (Slice.center.x=[" + Slice.center.x + "]) / DataGenerator.UNITY_GRID_UNIT=[" + DataGenerator.UNITY_GRID_UNIT + "]");
            }
            return x;
        }
        public float GetOffsetY(StringBuilder info_message)
        {
            ...
            y /= DataGenerator.UNITY_GRID_UNIT;//-1 * 
            if (info_message.Length < 10000)
            {
                info_message.AppendLine("GetOffsetY: y=[" + y + "] (( Collider.center.y=[" + Collider.center.y + "]) - (Slice.center.y=[" + Slice.center.y + "])) / DataGenerator.UNITY_GRID_UNIT=[" + DataGenerator.UNITY_GRID_UNIT + "]");
            }
            return y;
        }
        /// <summary>
        ...
        {
            return this.Collider.width / (float)DataGenerator.SLICE_WIDTH;
        }
        public float GetScaleY()
        {
            return this.Collider.height / (float)DataGenerator.SLICE_HEIGHT;
        }
    }
}
