...
");
            ...
            ColorBoxCondition[] boxesSettings = new ColorBoxCondition[]
            {
            ...
            foreach(ColorBoxCondition boxSettings in boxesSettings)
            {
                ...
                foreach (string filepath in filepaths)
                {
                    ...
                    Utility_BoxScanner.Execute_1Image(boxSettings, image_to_slice_to_rectangleList, filepath, info_message);
                    info_message.AppendLine("list.Count : " + image_to_slice_to_rectangleList.Count);
                }

                {
                    StringBuilder sb_dammy = new StringBuilder();
                    foreach (List<RectangleOnSlice> list in image_to_slice_to_rectangleList)
                    {
                        foreach (RectangleOnSlice rect in list)
                        {
                ...
                string text = Utility_Hitbox2DClassFormat.ToText(boxSettings.m_outputClassName, image_to_slice_to_rectangleList, info_message);
                System.Console.WriteLine(text);
                script.Append(text);
            }

            script.AppendLine("}");
            File.WriteAllText(DataGenerator.FILE_PATH, script.ToString());
